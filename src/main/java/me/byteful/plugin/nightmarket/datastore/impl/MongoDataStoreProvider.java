...
public class MongoDataStoreProvider implements DataStoreProvider {
    ...
    public void setPlayerShop(PlayerShop shop) {
        final MongoCollection<Document> col = this.getCollection();
    ...
    public Set<PlayerShop> getAllShops() {
        ...
        this.getCollection().find()
            .forEach((Consumer<? super Document>) doc -> set.add(
                DataStoreProvider.GSON.fromJson(doc.toJson(JsonWriterSettings.builder().outputMode(JsonMode.RELAXED).build()), PlayerShop.class)));
    ...
    public boolean test() {
        try {
            this.getCollection().find(Filters.eq("test", "test")).first();
            return true;
        } catch (Exception e) {
            this.logger.log(Level.SEVERE, "Failed to test MongoDB connection.", e);
            return false;
        }
    }

    private MongoCollection<Document> getCollection() {
        return this.db.getCollection("NightMarket");
    }
